    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 text-slate-800">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <section className="text-center max-w-3xl mx-auto">
          <h2 className="text-3xl md:text-4xl font-bold text-gray-900">Welcome to Moffat Bay</h2>
          <p className="mt-3 text-lg text-gray-600">
            Nestled along the stunning Pacific Northwest coast, Moffat Bay Marina &amp; Lodge offers adventure, relaxation, and family-friendly activities for everyone. From hiking trails to scenic kayaking routes, our attractions make for an unforgettable stay.
          </p>
        </section>
        ...
        <section className="mt-16 bg-white rounded-xl py-12 text-center shadow">
          <h2 className="text-2xl md:text-3xl font-bold text-gray-900">Ready to Explore Moffat Bay?</h2>
          <a href="/availability" className="inline-block mt-4 bg-indigo-600 text-white font-semibold px-6 py-3 rounded-full hover:bg-indigo-700 transition">Book Your Stay Now</a>
        </section>
      </main>
        ...
        >
          <div className="bg-white rounded-xl max-w-2xl w-full overflow-auto">
            <div className="relative">
              ...
              <img src={openItem.img} alt={openItem.title} className="w-full h-64 object-cover rounded-t-xl" />
            </div>
            <div className="p-6">
              <h3 className="text-sky-700 text-2xl font-semibold">{openItem.title}</h3>
              <p className="mt-3 text-sm text-slate-600">{openItem.desc}</p>
              <div className="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
                <div><strong>Distance:</strong><div className="text-slate-700">{openItem.distance}</div></div>
                <div><strong>Rates:</strong><div className="text-slate-700">{openItem.rates}</div></div>
                <div><strong>Times:</strong><div className="text-slate-700">{openItem.times}</div></div>
              </div>
            </div>
          </div>
        </div>
